...
EXIST_CURRENT=$(docker compose ${DOCKER_APP_NAME}-${CURRENT} -f ${DEPLOYPATH}prod/script/prod/docker-compose.${CURRENT}.yml ps  --status=running | grep ${DOCKER_APP_NAME}-${CURRENT}) 
EXIST_ALTER=$(docker compose ${DOCKER_APP_NAME}-${ALTER} -f ${DEPLOYPATH}prod/script/prod/docker-compose.${ALTER}.yml ps  --status=running | grep ${DOCKER_APP_NAME}-${ALTER}) 
DATE=$(date '+%Y%m%d%H%M%S') #배포날짜변수 
echo "DATE : $DATE"

if [ -z "$EXIST_CURRENT" ]; then #blue가 없을 때
    echo "$EXIST_CURRENT up start"   
    docker compose ${DOCKER_APP_NAME}-${CURRENT} -f ${DEPLOYPATH}prod/script/prod/docker-compose.${CURRENT}.yml up -d
    ...
    if [ -n "$EXIST_ALTER" ]; then # green이 떠있을때 
        ...
        docker compose ${DOCKER_APP_NAME}-${ALTER} -f ${DEPLOYPATH}prod/script/prod/docker-compose.${ALTER}.yml down
    ...
    docker compose ${DOCKER_APP_NAME}-${ALTER} -f ${DEPLOYPATH}prod/script/prod/docker-compose.${ALTER}.yml up -d
    sudo cp ${DEPLOYPATH}conf.d/server.${ALTER}.conf ${DEPLOYPATH}conf.d/nginx.server.conf
    docker exec front-web nginx -s reload
    echo "${DOCKER_APP_NAME}-${CURRENT} down start"
    docker compose ${DOCKER_APP_NAME}-${CURRENT} -f ${DEPLOYPATH}prod/script/prod/docker-compose.${CURRENT}.yml down
